...
class Optimizer(ABC):
	...
	def assign_fitness(self, fitness: Iterable[NDArray], *data: Optional[Iterable[Any]]) -> None:
		...
		), f"Error: {len(fitness)} Fitness values found, {self._num_objectives} was expected."
		...
		new_metrics = np.ascontiguousarray(
			fitness
		).T  # Fitness is shaped (num_objectives, num_candidates) transpose to get (num_candidates, num_objectives)
		...
		for i in range(metrics.shape[0]):
			...
			if i >= n:
				break
			on_pareto[i + 1 : n] = ((metrics[sorted_indices][i + 1 :] <= metrics[sorted_indices][i]).all(axis=1)
									& (metrics[sorted_indices][i + 1 :] < metrics[sorted_indices][i]).any(axis=1))
